...
export const DUPLICATE_CONFIG = {
  // Similarity thresholds for different sensitivity levels
  ...
  MAX_LINES_TO_SCAN: 10, // Only check first N lines for plain text titles
  PLAIN_TEXT_SCAN_LINES: 3, // Check first 3 lines for non-header titles
  
  // Patterns for detecting markdown headers
  MARKDOWN_HEADER_PATTERN: /^(#{1,6})\s+(.+)$/,
  
  // Content cleaning after removal
  MAX_CONSECUTIVE_EMPTY_LINES: 2,
  
  // Exact match threshold (for "only exact matches" setting)
  EXACT_MATCH_THRESHOLD: 0.95,
  
  // Minimum similarity for any detection
...
export const DUPLICATE_MESSAGES = {
  DUPLICATES_FOUND: 'Found {count} duplicate title(s) in note content',
  DUPLICATES_REMOVED: 'Removed {count} duplicate title(s) from note content',
  NO_DUPLICATES_FOUND: 'No duplicate titles found in note content',
  AUTO_REMOVAL_SKIPPED: 'Auto-removal disabled, duplicates not removed',
  USER_DECLINED_REMOVAL: 'User declined to remove duplicate titles',
  REMOVAL_FAILED: 'Failed to remove duplicate titles from content',
  CONFIRMATION_TITLE: 'Duplicate Titles Detected',
  CONFIRMATION_MESSAGE: 'Found {count} duplicate title(s) similar to "{title}" in the note content',
  CONFIRMATION_QUESTION: 'Would you like to remove these duplicate titles from the note content?',
} as const;